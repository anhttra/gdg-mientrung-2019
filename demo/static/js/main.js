...
function setStreamingEnable(enable){
    video = document.getElementById('video');
    var tracks = video.srcObject.getTracks();
    for(var i = 0; i < tracks.length; i ++){
        tracks[i].enable = enable;
    }
}
    ...
    getCanvasBlob(canvas).then(function(blob) {
        ...
        fetch(url, {
            ...
            if (json && json['error'] == 0 && json.data){
                let data = json.data;
                let str1 = (data.class===0)?"It's a cat":"It's a dog";
                let str2 = `Confidence:${data["confidence"]}%`;
                let str3 = `Predict take ${data["eslapse"].toFixed(2)} ms`;
                Swal.fire({
                    // type: 'success',
                    title: str1,
                    text: str2,
                ...
                let str = 'Error when predict!' + json.toString();
                Swal.fire({
                    type: 'error',
                    title: 'Oops...',
                    text: str,
